...
def parse_args():
    ...
    # ---------- 单张图片预测 ----------
    parser.add_argument("--count", default=True, type=bool)  # 指定了是否进行目标的像素点计数（即面积）与比例计算
    parser.add_argument("--img-path", default="./img/d_r_4_.jpg", type=str)
    parser.add_argument(
        "--img-save-path", default="./img_out/predict_img.png", type=str
    )

    # ---------- 多张图片预测 ----------
    parser.add_argument("--dir-origin-path", default="img/", type=str)
    parser.add_argument("--dir-save-path", default="img_out/", type=str)

    # ---------- fps计算模式 ----------
    parser.add_argument(
        "--test-interval", default=1000, type=int, help="image test interval"
    )
    parser.add_argument(
        "--fps-image", default="./img/d_r_4_.jpg", type=str, help="image root"
    )

    # ---------- 视频或摄像头预测 ----------
    parser.add_argument("--video-path", default="", type=str)
    parser.add_argument("--video-save-path", default="", type=str)
    parser.add_argument("--video-fps", default=25.0, type=float)
...
def main(args):
    ...
    # ----------------------------------------------------------------------------------------------------------#
    mode = args.mode
    # 区分的种类，和json_to_dataset里面的一样，用于打印种类和数量
    name_classes = [
