...
class ResNet(nn.Module):
    def __init__(
        ...
        if replace_stride_with_dilation is None:
            ...
            replace_stride_with_dilation = [False, False, False]  # 普通卷积
        ...
        self.conv1 = nn.Conv2d(
            in_channels=3, out_channels=self.inplanes, kernel_size=7, stride=2, padding=3, bias=False
        )
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))  # 自适应全局平均池化卷积
    ...
    def _forward_impl(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(input=x, dims=1)
        x = self.fc(x)
